...
export default function Login_page({ onNavigate }) {
  const { currentUser, login, loginDemo, loginWithGoogle, loginWithGithub } = useAuth();
  ...
  // Nếu người dùng đã đăng nhập -> Chặn truy cập và tự động chuyển về trang chủ
  useEffect(() => {
    if (currentUser) {
      addToast(`Bạn đã đăng nhập (${currentUser.name || currentUser.username})! Đang chuyển về trang chủ...`, "info");
      ...
      <div className="min-h-screen flex items-center justify-center bg-base-200 p-4">
        <div className="card bg-base-100 p-8 rounded-3xl shadow-xl text-center max-w-md w-full space-y-4 animate-fade-in border border-base-300">
          ...
          <p className="text-sm text-base-content/70">
            Tài khoản: <strong className="text-primary">{currentUser.name || currentUser.username}</strong>
          </p>
          <p className="text-xs text-base-content/50">
            Trang đăng nhập chỉ dành cho khách chưa đăng nhập. Đang tự động chuyển hướng về trang chủ...
          </p>
          <button
            type="button"
