        ...
        {items.map((h) => (
          <li key={h.id}>
            <button
              ...
              onClick={() => go(h.id)}
              className={
                "w-full text-left text-xs sm:text-sm rounded-lg px-2 py-1.5 hover:bg-primary/10 hover:text-primary transition-colors flex items-start " +
                (h.level === 1 ? "font-black" : h.level === 2 ? "font-bold ml-1" : "ml-4 text-base-content/80")
              }
            >
              <span className="text-primary font-bold mr-1.5 shrink-0 mt-0.5">
                {h.level === 1 ? "●" : h.level === 2 ? "◆" : "○"}
              </span>
              <span className="break-words line-clamp-2">{h.text}</span>
            </button>
          </li>
        ))}
      </ul>
